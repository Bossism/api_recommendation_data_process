...
public class CFGBuilder {
	...
	public ControlFlow switchStmt(final Vertex v, final List<ControlFlow> flows) {
		final Set<Vertex> out = new HashSet<>();
		for (ControlFlow flow : flows) {
			ControlFlow connect = connect(v, flow);
			out.addAll(connect.getOut());
		}
		final ControlFlow result = new ControlFlow(v, out);
		for (ControlFlow flow : flows) {
			result.getBreaks().addAll(flow.getBreaks());
		}
		return result;
	}
	...
	public ControlFlow switchEntry(final Vertex v, final ControlFlow switchEntryFlow) {
		return connect(v, switchEntryFlow);
	}
