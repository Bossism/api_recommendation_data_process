...
public class VertexCreator {
	...
	public Vertex switchStmt(final SwitchStmt n) {
		final Expression cond = n.getSelector();
		final String label = cond.toString();
		final Vertex result = new Vertex(VertexType.CTRL, label, cond);
		setId(result);
		setLine(result, n);
	...
	public Vertex switchEntry(final SwitchEntryStmt n) {
		final String label = n.getLabel().toString();
